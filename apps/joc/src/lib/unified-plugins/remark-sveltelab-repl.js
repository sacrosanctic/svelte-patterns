 ...
 */
export function remarkSveltelabRepl() {
	/**
	...
	return (tree, file) => {
		visit(tree, 'paragraph', (node, index, parent) => {
			...
			if (!textNode.value.startsWith(':::sveltelab-repl')) return
			const markdownDir = path.dirname(file.filename)

			const files = parseGroup(node, 'sveltelab-repl')
				.map(({ name, filepath }) => ({
					name,
			...
			if (paragraphNode?.type === 'paragraph') {
				paragraphNode.children = [
					{ type: 'html', value: `<SveltelabRepl files={${JSON.stringify(files)}} />` },
				]
			}

			upsertScript(tree, 'SveltelabRepl', '@repo/ui')
		})
	}
}
