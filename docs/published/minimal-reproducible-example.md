...
export async function GET({ url }) {
	try {
		...
		const rows = statements.getClifor_gen_paginated_search(cliforType, searchQuery, limit, offset)
...
_(Don't click the source if you want to try solving this yourself)_  
...
<button onclick="{finishTodo}" class:line-through="{todo.ready}">{todo.title}</button>
